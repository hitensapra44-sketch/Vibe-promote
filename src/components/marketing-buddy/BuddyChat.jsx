...
export default function BuddyChat() {
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      autoMarkTask(user.id, '/buddy-chat');
      autoMarkTask(user.id, '/marketing-buddy');
      ...
      <div className="bg-background border-t border-foreground/10 pt-4 space-y-3">
        <div className="flex flex-wrap gap-2">
          {suggestions.map((prompt) => (
            <button
              ...
              disabled={isTyping}
              className="bg-foreground/5 border border-foreground/10 text-zinc-500 text-[10px] font-bold rounded-full px-3 py-1.5 text-left hover:border-zinc-600 hover:text-foreground transition-all disabled:opacity-50 cursor-pointer"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="flex gap-2 items-end">
          ...
          <button 
            ...
            className={cn(
              "p-3 rounded-xl transition-all flex-shrink-0 border-none cursor-pointer",
