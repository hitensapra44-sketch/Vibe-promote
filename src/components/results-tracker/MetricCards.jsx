  ...
  if (!metrics) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-foreground/5 border border-foreground/10 rounded-xl p-5 animate-pulse">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {items.map((item) => {
        ...
        return (
          <div key={item.key} className="bg-background border border-foreground/10 rounded-xl p-5 shadow-sm">
