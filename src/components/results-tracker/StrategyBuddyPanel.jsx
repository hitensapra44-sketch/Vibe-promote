...
export default function StrategyBuddyPanel({ analysis, isLoading }) {
  ...
  return (
    <div className="bg-background border-l-4 border-l-orange-500 border border-foreground/10 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-foreground text-sm font-semibold">Strategy Buddy</h3>
            <p className="text-zinc-500 text-[10px] uppercase tracking-widest">Based on this week's data</p>
          </div>
        </div>
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-foreground/10 text-zinc-500 text-[10px] font-bold hover:border-zinc-800 transition-all bg-transparent">
          <RefreshCw size={12} />
          Refresh Analysis
        </button>
      </div>
        ...
        <div className="space-y-8">
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-green-600">
              ...
              <span className="text-sm font-medium text-foreground">What's Working</span>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-yellow-600">
              <AlertCircle size={16} />
              <span className="text-sm font-medium text-foreground">What to Improve</span>
            </div>
            <p className="text-zinc-700 text-sm leading-relaxed pl-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-orange-500">
                <Calendar size={16} />
                <span className="text-sm font-medium text-foreground">plan fo next week according to current analytics</span>
              </div>
              <button 
              ...
              {isGeneratingStrategy ? (
                ...
                currentPlan.split('\n').map((line, i) => (
                  <div key={i} className="flex gap-2 text-sm">
                    ...
                    <span className="text-zinc-700">{line.replace('→ ', '')}</span>
                  </div>
                ))
              ) : null}
            </div>
          ...
          <div className="pt-8 border-t border-foreground/10 space-y-4">
            <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Ask about this week</p>
            <div className="flex gap-2">
              <input 
                type="text" 
                placeholder="e.g. Why did my LinkedIn drop?"
                className="flex-1 bg-foreground/5 border border-foreground/10 rounded-lg px-4 py-2.5 text-sm text-foreground focus:outline-none focus:border-orange-500 placeholder-zinc-500"
                value={question}
            ...
            {answer && !isAnswering && (
              <div className="bg-foreground/5 border border-foreground/10 rounded-lg p-4 text-zinc-700 text-sm leading-relaxed">
                {answer}
              </div>
            )}
          </div>
