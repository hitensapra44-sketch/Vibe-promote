...
export default function AudienceSpotter() {
  ...
  useEffect(() => {
    async function fetchBrain() {
      ...
      try {
        ...
        if (data) {
          setBrain(data);
          // Only auto-skip to monitoring if they have signals already found, 
          // meaning they've definitely completed setup before.
  ...
  const handleCreateSignal = async () => {
    ...
    if (error) {
      ...
      setIsConfigured(true);
      startScan(); // Trigger the backend scraper only now
    }
  };
                ...
                {STEPS.map((s) => (
                  <div key={s.id} className="relative flex items-center gap-4">
                    <div className={cn(
                      ...
                      step === s.id ? "bg-[#E11D48] border-[#E11D48] text-white shadow-lg shadow-[#E11D48]/20" : 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {PLATFORMS.map((p) => (
                        ...
                        >
                          ...
                          <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center transition-colors", selectedPlatforms.includes(p.id) ? "bg-[#E11D48]" : "bg-zinc-800")}>
                  ...
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {keywords.map((k, i) => (
                          <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#E11D48]/10 border border-[#E11D48]/20 text-[#E11D48] text-xs font-bold group">
                      ...
                      <div className="p-6 rounded-2xl bg-zinc-900/40 border border-zinc-800 flex items-center justify-between">
                        <div className="flex gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-zinc-500">Summary when new signals are found.</p>
                          </div>
                        </div>
                        <div 
          ...
          {signals.length === 0 ? (
            ...
            <div className="space-y-6 pb-20">
              <div className="flex items-center justify-between mb-8">
                 <div className="flex items-center gap-2">
                    <span className="bg-[#E11D48] text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider">Live</span>
              ...
              {signals.map((signal) => (
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="bg-[#1A1A1A] border-l-4 border-[#E11D48] rounded-r-2xl p-5 mb-8 group-hover:bg-white/[0.03] transition-colors">
                      <div className="flex items-center gap-2 mb-3">
                        <MessageSquare className="w-3.5 h-3.5 text-[#E11D48]" />
                        <span className="text-[10px] font-bold text-[#E11D48] uppercase tracking-widest">Suggested Reply</span>
                      </div>
