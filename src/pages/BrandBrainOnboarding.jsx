...
Return ONLY valid JSON. No markdown, no extra text.`;
    ...
    try {
      ...
      // If extraction fails, we still show the form so they can fill manually
      setHasExtracted(true);
    } finally {
      setExtracting(false);
    }
    ...
    <div className="relative min-h-screen bg-bg-base text-white font-poppins overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 pt-24 pb-20">
        ...
        >
          ...
          <h1 className="text-4xl sm:text-6xl font-bold text-white mb-6" style={{ letterSpacing: '-2px' }}>
            Let AI build your <br />
            <span className="text-primary">marketing brain.</span>
          </h1>
          
          {!hasExtracted ? (
            <div className="max-w-xl mx-auto">
              <p className="text-text-secondary mb-8 text-lg">Paste your URL. We'll handle the rest.</p>
                ...
                <div className="relative flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-5 rounded-2xl bg-bg-surface/80 backdrop-blur-xl border border-white/10 text-white focus:outline-none focus:border-primary/50 transition-all"
                      required
                    />
                  </div>
                  <button
                    ...
                    className="px-8 py-5 rounded-2xl bg-primary hover:bg-primary-hover text-white font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-primary/20"
                  >
                    {extracting ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Sparkles className="w-5 h-5" /> Analyze</>}
                  </button>
                </div>
              </form>
              <button 
                onClick={() => setHasExtracted(true)}
                className="mt-6 text-sm text-text-secondary hover:text-white transition-colors underline underline-offset-4"
              >
                Or fill details manually
              </button>
            </div>
          ) : (
            <p className="text-text-secondary">We've extracted your details. Feel free to tweak them.</p>
          )}
        </motion.div>
          ...
          >
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`w-full px-6 py-4 rounded-xl bg-bg-surface/50 border ${errors.app_name ? 'border-red-500' : 'border-white/5'} text-white focus:outline-none focus:border-primary/50 transition-all`}
                />
              </div>
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className={`w-full px-6 py-4 rounded-xl bg-bg-surface/50 border ${errors.app_description ? 'border-red-500' : 'border-white/5'} text-white focus:outline-none focus:border-primary/50 transition-all resize-none`}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`w-full px-6 py-4 rounded-xl bg-bg-surface/50 border ${errors.target_customer ? 'border-red-500' : 'border-white/5'} text-white focus:outline-none focus:border-primary/50 transition-all`}
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`w-full px-6 py-4 rounded-xl bg-bg-surface/50 border ${errors.core_problem ? 'border-red-500' : 'border-white/5'} text-white focus:outline-none focus:border-primary/50 transition-all`}
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-6 py-4 rounded-xl bg-bg-surface/50 border border-white/5 text-white focus:outline-none focus:border-primary/50 transition-all"
                />
              </div>
            </div>
            ...
            <div className="md:col-span-2 pt-6 flex justify-center">
              <button
                ...
                className="group inline-flex items-center gap-3 px-12 py-5 rounded-2xl bg-primary hover:bg-primary-hover text-white font-bold text-lg transition-all shadow-xl shadow-primary/20"
              >
